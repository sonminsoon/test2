						...
						<table summary="">
							...
							<tbody>
								<tr>
									...
									<? if ($nid_ClientID) { ?><td rowspan="2" valign="top"><?=$naver->login()?></td><? } ?>
									<? if ($fb_appId) { ?><td rowspan="2" valign="top"><a href="javascript:facebookLogin()"><img src="images/login_facebook.gif" alt="페이스북로그인"></a></td><? } ?>
									<? if ($kakao_secret) { ?><td rowspan="2" valign="top"><a href="javascript:loginWithKakao()"><img src="images/login_kakao.gif" alt="카카오톡로그인"></a></td><? } ?>
								</tr>
								<tr>
									<th>비밀번호</th>
								...
								<tr>
									...
									<td></td>
									<? if ($nid_ClientID) { ?><td></td><? } ?>
									<? if ($fb_appId) { ?><td></td><? } ?>
									<? if ($kakao_secret) { ?><td></td><? } ?>
								</tr>
							</tbody>
						</table>
					</form>
...
<? } else { echo "<script>window.parent.location.href='/';</script>"; } ?>

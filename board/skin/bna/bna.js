$(document).ready(function(){
	...
	if(document.getElementById('bnf_wrap')){
		...
		big_btn.on('mouseenter',function(e){clearInterval(bnf_id); chk=$(this).index(); if(over!=chk){res=over;over=chk; activeEvent();}});
		big_btn.on('mouseleave',function(e){bnfPlay();});
		...
		function off_img(){
			if(sum_btn.length<=4){
				$('#bnf_wrap .next').css({top:620});
				$('#bnf_wrap .prev').css({top:620});
			}else{
				$('#bnf_wrap .next').css({top:620});
				$('#bnf_wrap .prev').css({top:620});
			}
			...
			sum_btn.each(function(i){
				$(this).find('img').stop().animate({opacity:.5},0);if(i%4 ==3){$(this).css({'margin-right':0})}})
			...
			sum_btn.on('click',function(e){
				...
				$.get('/board/ajax.board_bna.php?number='+number+'&tb='+tblname,
					function(data,state){
						...
						$('.view_box .btn li').on('mouseenter',function(e){clearInterval(bnf_id); chk=$(this).index(); if(over!=chk){res=over;over=chk; activeEvent();}});
						$('.view_box .btn li').on('mouseleave',function(e){bnfPlay();});
					}
				);	

...
<script language="javascript">
	function fnConfirm(p_oForm) {
		<? if(!$_SESSION["ss_id"] && $_GET['act']=='write'){   ?>
		...
		<? } ?>
		if (!p_oForm.strMobile2.value){
			alert("연락처를 입력하셔야 합니다");
			p_oForm.strMobile2.focus();
			return false;
		}
		if (!p_oForm.strMobile3.value){
			alert("연락처를 입력하셔야 합니다");
			p_oForm.strMobile3.focus();
			return false;
		}
		if(p_oForm.wr_key){
...
<?   if(!$_SESSION["ss_id"] && $_GET['act']=='write'){   ?>
	...
	<div class="agreeBox">
		...
		<p><input name="agree" id="agree" type="checkbox" value="" required>&nbsp;<label for="agree">개인정보취급방침에 동의합니다</label></p>
	</div>
	<!----//개인정보를 받는 모든 게시판에 개인정보취급방침 동의---->
<?	}	?>

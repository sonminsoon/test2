...
define("root" ,$_SERVER['DOCUMENT_ROOT']);
	...
	include $_SERVER['DOCUMENT_ROOT']."/include/lib/board.lib.php";
	...
	if( !$act || $act == "list" ) {
		...
		while( $Array = mysql_fetch_array( $Sql ) ) {
			...
			$Data[$lTmp]["tblStrComment2"]		= mb_strimwidth( preg_match("/<[^>]*>/", "", stripslashes( $Array["tblStrComment2"] ) ), 0, 100, "...", "utf-8" );
			...
			/* 제목길이 조절 시작 */
			$tCuts = ( $boardSet["skin"] == 'counsel' ) ? "70" : "70";
			...
			if( $boardSet["skin"] != "question" ) {
				if ( $_SESS["ss_level"] <= 2 || ( $Data[$lTmp]["secret"] == 'N' && $_SESS["ss_level"] <= $boardSet["viewlevel"] ) || ( $_SESS["ss_name"] && $_SESS["ss_id"] == $Data[$lTmp]["id"] ) ) {
					$Data[$lTmp]["viewlink"] = "<a href='".$PHP_SELF."?tb=".$tb."&act=view&tNum=".$Data[$lTmp]["number"]."&sField=$sField&p=$p'>";
				} else {
				...
				if(($tb=='review2') && ($_SESS["ss_id"]=='' || !$_SESS["ss_id"])){
					 $Data[$lTmp]["viewlink"]="<a href='javascript:mem_login();'>";
				}
			}else {
	...
	if( $act == 'write_ok' ) {
			...
			// 분류를 다중으로 받을경우
			if (is_array($_POST["intField"])) {
				$_POST["intField"] = "[".implode("][", $_POST["intField"])."]";
			} else {
				$_POST["intField"] = ($_POST["intField"])?"[".$_POST["intField"]."]":'';
			}

			// 전문의료상담은 무조건 비밀글
			//if ($tb == 'online_counsel') $_POST["strSecret"] = 'Y';
			...
			$Data["thumfile1"]		= ( $Data["thumfile1"] ) ? implode( '|', $Data["thumfile1"] ) : "";
		...
		if($tb == 'event'){
			$iQue .= " entSubject='".$_POST["entSubject"]."',";
			$iQue .= " entStart='".$_POST["entStart"]."',";
			$iQue .= " entEnd='".$_POST["entEnd"]."',";
		}
	...
	if( $act == 'modify' ) {
		...
		if($Data["secret"] == 'Y' && $_SESS["ss_level"] > 2){
			if($_SESS["ss_id"] != $Data["id"] || $_SESS["ss_id"]==''){
				if($bibun != $Data["passwd"] || $bibun==''){
					echo "<script language='javascript'>
						alert('비밀글입니다. 비밀번호를 입력하여 주세요.'); 
						location='".$_SERVER['PHP_SELF']."?tb=".$tb."&act=passwd&act_after=modify&tNum=".$tNum."';
					</script>";
					exit;
				}
			}
	...
	if( $act == 'modify_ok' ) {
		...
		for( $j = 0; $j < $boardSet["addfilenumber"]; $j++ ) {
			...
			if( trim( $Data["savefile"][$j] ) ) { 
				...
				if( $Modify["savefile"][$j] ){
					@unlink( $_SERVER['DOCUMENT_ROOT']."/_data/".$tb."/".$Modify["savefile"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT']."/_data/".$tb."/".$Modify["liefile"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT']."/_data/".$tb."/".$Modify["thumfile1"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT']."/_data/".$tb."/".$Modify["thumfile2"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT']."/_data/".$tb."/".$Modify["thumfile3"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT']."/_data/".$tb."/".$Modify["thumfile4"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT']."/_data/".$tb."/".$Modify["thumfile5"][$j] );

					@unlink( $_SERVER['DOCUMENT_ROOT'].$Modify["savefile"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Modify["liefile"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Modify["thumfile1"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Modify["thumfile2"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Modify["thumfile3"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Modify["thumfile4"][$j] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Modify["thumfile5"][$j] );
				} 
				/* 이미지 정리 시작 */
		...
		// 분류를 다중으로 받을경우
		if (is_array($_POST["intField"])) {
			$intField = "[".implode("][", $_POST["intField"])."]";
		}
		...
		$mData["comment"]		= addslashes( $strComment );
		...
		if( $boardSet["control"] == 'Y' ) {
			$mData["show"]		= ( $_POST["strShow"] == 'Y' ) ? "Y" : "N";
		} else {
			$mData["show"]		= "Y";
		}
		...
		if( $_SESS["ss_level"] <= 2 && $mData["newemail"] == 'Y' && trim( $mData["reply"] ) ) {
			...
			$headers .= "\n"; 
			$m_contents = sock_url( $bagData["host"], "80", "http://".$bagData["host"]."/mail/letter_counsel.html");
			...
			$m_contents = str_replace("@ETC1@", $mData["comment"], $m_contents);
			$m_contents = str_replace("@ETC2@", $mData["reply"],$m_contents);
	...
	if( $act == 'view' ) {		
		...
		if($Data["secret"] == 'Y' && $_SESS["ss_level"] > 2){
			if($_SESS["ss_id"] != $Data["id"] || $_SESS["ss_id"]==''){
				if($bibun != $Data["passwd"] || $bibun==''){
					echo "<script language='javascript'>
							alert('비밀글입니다. 비밀번호를 입력하여 주세요.'); 
							location='".$_SERVER['PHP_SELF']."?tb=".$tb."&act=passwd&act_after=view&tNum=".$tNum."';
						</script>";
					exit;
				}
			}
		...
		if ($tb == "online_counsel") {
			if ( $_SESS["ss_level"] <= 2) {
				$view["replylink"] = "<a href='".$PHP_SELF."?tb=".$tb."&act=reply&tNum=".$Data["number"]."'>";
			}
		} else {
		...
		for( $fileCnt = 0; $fileCnt < count( $Data["savefile"] ); $fileCnt++ ) { 
			if( $Data["savefile"][$fileCnt] ) {
				...
				$Data["imgSize"] = @getImageSize($_SERVER['DOCUMENT_ROOT'].$saveFileStr);

				$Data["file_img"][$fileCnt] = ( $Data["savefile"][$fileCnt] ) ? $Data["savefile"][$fileCnt] : "/board/skin/".$boardSet["skin"]."/images/noimg_big.gif";

				// 썸네일 기준으로 찾고 없으면 원본 파일 검색
				if ( $Data["thumfile1"][$fileCnt] ) {
					$Data["filename2"] = $Data["thumfile1"][$fileCnt];
				} else if ( $Data["savefile"][$fileCnt] ) {
					$Data["filename2"] = $Data["savefile"][$fileCnt];
				} else {
					$Data["filename2"] = "/board/skin/".$boardSet["skin"]."/images/noimg_big.gif";
				}

				// 다운로드 이미지 세팅
				$Data["downimg"][$fileCnt] = "<a href='/board/download.php?tb=".$tb."&FileName=".str_replace("/_data/".$tb."/", "", $Data["savefile"][$fileCnt])."&wn=".urlencode( $Data["liefile"][$fileCnt] )."'>".$Data["filename"]."</a>";

				// 미리보기 이미지 세팅
				$Data["openimg"][$fileCnt] = "<img src='".$Data["filename2"]."' ".$Data["WidthHeight"]." name='att_img' id='att_img'>";
			}else{
	...
	if( $act == 'reply_ok' ) {
		...
		/* 자동글등록방지 체크 시작 */
		if($_POST['wr_key'] && $boardSet["skin"] != 'call'){
			include $_SERVER['DOCUMENT_ROOT']."/norobot/norobot_check.inc.php";
		}
		/* 자동글등록방지 체크 끝   */
		
		$mData["reply"]	= addslashes( $_POST['strReply'] );
		$mData["newemail"] = $_POST['newemail'];

		$row = mysql_fetch_array(mysql_query("SELECT * FROM tbl_$tb WHERE tblNumber='$tNum'"));

		/* 기존 데이타 답변 업데이트 시자 */
		$rQue = "UPDATE tbl_".$tb." SET ";
		$rQue .= "tblStrReply='".$mData["reply"]."',";
		$rQue .= "tblStrReID='".$_SESS["ss_id"]."',";
		$rQue .= "tblStrReName='".$_SESS["ss_name"]."' WHERE tblNumber='".$tNum."'";
		...
		if( $_SESS["ss_level"] <= 2 && $mData["newemail"] == 'Y' && trim( $mData["reply"] ) ) {
			...
			$m_contents = sock_url( $bagData["host"], "80", "http://".$bagData["host"]."/mail/letter_counsel.html");
			$m_contents = str_replace("@HOMEURL@",$homeUrl,$m_contents);
			$m_contents = str_replace("@NAME@",$row["tblStrName"],$m_contents);
			$m_contents = str_replace("@ETC1@", stripslashes( $row["tblStrComment"] ), $m_contents);
			$m_contents = str_replace("@ETC2@", $mData["reply"],$m_contents);
			...
			$title = stripslashes( $mData["subject"] );
			$title='=?UTF-8?B?'.base64_encode($title).'?=';
			$res = mail($row["tblStrEmail"], $title, $m_contents, $headers);
		}
		/* 메일발송 끝 */
	...
	if( $act == 'delete' ) {
		...
		$Data["subject"]	= $result["tblStrSubject"];

		// 내용에 있는 이미지 삭제(pc용 절대 경로)
		$src = imgTag($result["tblStrComment"]);
		if (is_array($src)) {
			foreach($src as $k => $v) {
				@unlink( $_SERVER["DOCUMENT_ROOT"].str_replace("\\","",$v) );
			}
		}
		...
		if( $Data["thread"]		== 'A' ) {
			...
			for( $i = 0; $i < count( $Data["savefile"] ); $i++ ) {
				if( $Data["savefile"][$i] ){
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Data["savefile"][$i] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Data["thumfile1"][$i] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Data["thumfile2"][$i] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Data["thumfile3"][$i] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Data["thumfile4"][$i] );
					@unlink( $_SERVER['DOCUMENT_ROOT'].$Data["thumfile5"][$i] );
				}
			}
		}
...
<script language='javascript'>
	...
	function mem_login(){
		...
		location.href = "/member/login.php?ref=<?=$PHP_SELF;?>";
	}
</script>